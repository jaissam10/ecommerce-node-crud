...
let addProduct = (req, res) => {
    ...
    resHandler.Validator(req.body, 'name', 'description', 'quantity', 'price').then(async (resValid) => {
        try {
            ...
            // } else {
                let product = new productModel({name: req.body.name, description: req.body.description, price: Number(req.body.price),quantity: Number(req.body.quantity)})
    ...
    */
    resHandler.Validator(req.body, '_id').then(async (resValid) => {
        try {
            ...
            let data = await productModel.findOneAndUpdate({_id: req.body._id, status: 'ACTIVE'}, {$set: query}, {lean: true, new: true});
            if(!data) {
...
let deleteProduct = (req, res) => {
    ...
    .then(async (resValid) => {
        try {
           ...
           let data = await productModel.findOneAndUpdate({_id: req.body._id}, query, {lean: true, new: true})
            if(!data) {
                return resHandler.ResponseWData(res, resCode.NOT_FOUND, resMsg.NOT_FOUND);
            } else {
